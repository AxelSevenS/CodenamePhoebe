...
namespace SeleneGame {
    ...
    public abstract class State : MonoBehaviour{
        ...
        protected void RotateEntity(Vector3 newRotation){
            ...
            entity._t.rotation = Quaternion.Slerp(entity._t.rotation, entity.apparentRotation * turnDirection, 12f*Time.deltaTime);
        }
        ...
        protected void RawInputToGroundedMovement(Entity entity, out Vector3 camRight, out Vector3 camForward, out Vector3 groundDirection, out Vector3 groundDirection3D){
            camRight = entity.currentFrameLookRotation * Vector3.right;
            camForward = Vector3.Cross(camRight, entity._t.up).normalized;
            groundDirection = (entity.currentFrameMoveInput.x * camRight + entity.currentFrameMoveInput.z * camForward).normalized;
            groundDirection3D = groundDirection + ((entity.currentFrameLookRotation * Vector3.up) * entity.currentFrameMoveInput.y);
        }
    }
}